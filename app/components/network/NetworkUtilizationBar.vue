<template>
  <div class="space-y-1.5">
    <div class="flex h-2 overflow-hidden rounded-full bg-neutral-200 dark:bg-neutral-700">
      ...
      <div v-if="usedPrefixRangePercent > 0" class="h-full bg-violet-500/70 transition-all" :style="{ width: `${usedPrefixRangePercent}%` }" />
      <!-- DHCP ranges -->
    ...
    <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-gray-400">
      ...
      <span v-if="usedPrefixRangePercent > 0" class="flex items-center gap-1"><span class="inline-block h-2 w-2 rounded-full bg-violet-500/70" /> {{ $t('networks.ranges.types.used_prefix') }} ({{ usedPrefixIpCount }} IPs)</span>
